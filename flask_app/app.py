...
def dashboard():
    if request.method == 'GET':
        ...
        cur.execute(
            'SELECT date desc from sensordata where date >= current_date() - 7 group by date')
    ...
    if request.method == 'POST' and request.form['loadButton'] == 'Laden':
        ...
        cur.execute('''SELECT date asc FROM sensordata where date >= (%s) and id_satellite_programm in 
